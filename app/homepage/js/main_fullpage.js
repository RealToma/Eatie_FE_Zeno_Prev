...
gsap.set('.map_group', { autoAlpha: 1, rotate:30});
gsap.set('.map_bg', { autoAlpha: 0 });
gsap.set('.connect_bread', { top:'45vh',right:'-75vw', autoAlpha: 1});
...
function slideIn() {
  ...
  switch(next) {
    case 0:
      // index
      console.log("0 begin current:"+current+" next:"+next);
      ...
      mm.add({
        ...
        gsap.timeline({
          ...
          onComplete: () => {
            listening = true;
            current = next;
            console.log("0 End current:"+current+" next:"+next);
          }
        })
        .to('.logo', {
          y: isDesktop ? '35vh' : '14vh', 
    ...
    case 1:
      ...
      mm.add({
        ...
        //.to(about_item, {duration:1.5, stagger:0.2, autoAlpha: 1, y:'30vw', ease:"back"})
    ...
    case 3:
      ...
      mm.add({
        ...
        gsap.timeline({
          onComplete: () => {
            console.log("3 onComplete current:"+current+" next:"+next);
            gsap.set(sections[current], { autoAlpha: 0, visibility:false, zIndex: 0 });
            listening = true;
            current = next;
            console.log("3 End current:"+current+" next:"+next);
          }
        })
    ...
    case 4:
        ...
        console.log("4 begin current:"+current+" next:"+next);
        ...
        mm.add({
          ...
          gsap.timeline({
            onComplete: () => {
              console.log("4 onComplete current:"+current+" next:"+next);
              gsap.set(sections[current], { autoAlpha: 0, visibility:false, zIndex: 0 });
              listening = true;
              current = next;
              console.log("4 End current:"+current+" next:"+next);
            }
          })
          ...
          .to(color_obj, { backgroundColor:'#FFFFFF' }, "<")
          .to(color_wheel, { stroke:'#FFFFFF' }, "<")
          .to('.map_group', { duration:0.8, autoAlpha: 1, rotate:isDesktop ? '0deg' : '-40deg' , ease: "power2.out" })
          .to(contents[current], { autoAlpha: 1 });
          return () => {
          }
        });
    ...
    case 5:
      //map > connect
        ...
        console.log("5 begin current:"+current+" next:"+next);
        
        gsap.set(sections[current], { zIndex: 0 });
        gsap.set(sections[next], { autoAlpha: 1, visibility: true, zIndex: 8 });
        gsap.set(contents[current], { autoAlpha: 0, visibility: true });
        //gsap.set('.map_bg, .main_bg .map_bg::after', { zIndex: 0});
        ...
        mm.add({
          ...
          gsap.timeline({
            ...
            duration: 1,
            onComplete: () => {
              console.log("5 onComplete current:"+current+" next:"+next);
              //gsap.set(sections[current], { autoAlpha: 0, visibility:false, zIndex: 0 });
              listening = true;
              current = next;
              console.log("5 End current:"+current+" next:"+next);
            }
          })
          .to('.map_group', { bottom:'-40vh', rotate:'90deg', autoAlpha: 0 }, 0)
          .to('.connect_bread', {
            top: isDesktop ? '40vh' : '60vh',
  ...
  switch(next) {
    case 0:
      ...
      mm.add({
        ...
        .to(color_obj, { backgroundColor:'#FFFFFF' }, "<")
        .to(color_wheel, { stroke:'#FFFFFF' }, "<")
        .to('.logo', {y:isDesktop ? '35vh' : '14vh',scale:isDesktop ? 1.8 : 1.4})
        ...
        gsap.timeline({
          ...
          duration: 0.8,
          onComplete: () => {
            console.log("0 onComplete current:"+current+" next:"+next);
            //gsap.set(sections[current], { autoAlpha: 0, visibility:false, zIndex: 0 });
            listening = true;
            current = next;
            console.log("0 End current:"+current+" next:"+next);
        ...
        .to('.slogan', {y:isDesktop ? '46vh' : '24vh', autoAlpha:1, duration:1.2})
        .to('.slide_show', { y:'0vh',duration: 0.8}, "-=0.6");
        return () => {
        }
      });
    ...
    case 1:
      ...
      //gsap.set(contents[next], { visibility: true, zIndex: 2 });
      mm.add({
        isDesktop: `(min-width: ${breakPoint}px)`,
        isMobile: `(max-width: ${breakPoint - 1}px)`,
        reduceMotion: "(prefers-reduced-motion: reduce)"
    ...
    case 2:
      // nft > play
      addClass('#scroll_wheel','percent_40');
      console.log("2 OUT 01 current:"+current+" next:"+next);
      ...
      mm.add({
        ...
        gsap.timeline({
          ...
          duration: 1,
          onComplete: () => {
            console.log("2 OUT onComplete current:"+current+" next:"+next);
            gsap.set(sections[current], { autoAlpha: 0, visibility:false, zIndex: 0 });
            listening = true;
            current = next;
            console.log("2 OUT End current:"+current+" next:"+next);
          }
        })
        .to(contents[next], { autoAlpha: 0 }, 0)
    ...
    case 3:
      // map > nft
      addClass('#scroll_wheel','percent_60');
      console.log("3 OUT 01 current:"+current+" next:"+next);
      ...
      mm.add({
        ...
        gsap.timeline({
          ...
          duration: 1,
          onComplete: () => {
            console.log("3 OUT onComplete current:"+current+" next:"+next);
            gsap.set(sections[current], { autoAlpha: 0, visibility:false, zIndex: 0 });
            gsap.set('.map_bg', { autoAlpha: 0 });
            listening = true;
            current = next;
            console.log("3 OUT End current:"+current+" next:"+next);
          }
        })
        .to(contents[next], { autoAlpha: 0 }, 0)
    ...
    case 4:
      ...
      console.log("map");
      addClass('#scroll_wheel','percent_80');
      console.log("4 OUT 01 current:"+current+" next:"+next);
      ...
      mm.add({
        ...
        gsap.timeline({
          ...
          duration: 1,
          onComplete: () => {
            console.log("4 OUT onComplete current:"+current+" next:"+next);
            gsap.set(sections[current], { autoAlpha: 0, visibility:false, zIndex: 0 });
            listening = true;
            current = next;
            console.log("4 OUT End current:"+current+" next:"+next);
          }
        })
        .to(contents[next], { autoAlpha: 0 })
        .to('.connect_bread', {
          top: isDesktop ? '45vh' : '60vh',
          right:isDesktop ? '-100vw' : '-100vw',
        })
        .to('.map_group', { bottom:isDesktop ? '-125vh' : '-62vh',rotate:isDesktop ? '0deg' : '-40deg', autoAlpha: 1})
        .to(contents[next - 1], { autoAlpha: 1 });
        return () => {
        }
      });
